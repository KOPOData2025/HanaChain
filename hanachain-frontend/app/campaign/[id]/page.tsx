                  ...
                  <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <span className="ml-2">{translatedCategory}</span>
                      </div>
                      <div>
                      ...
                      {campaign.blockchainStatus === 'ACTIVE' && (
                        <>
                          <div className="md:col-span-2">
                            <span className="text-gray-600">블록체인 상태:</span>
                            <span className="ml-2 text-[#009591] font-semibold">✓ 블록체인으로 검증됨</span>
                          </div>
                          {campaign.blockchainCampaignId && (
                            <div>
                              <span className="text-gray-600">블록체인 캠페인 ID:</span>
                              <span className="ml-2 font-mono text-[#009591]">#{campaign.blockchainCampaignId}</span>
                            </div>
                          )}
                          {campaign.blockchainTransactionHash && (
                            <div>
                              <span className="text-gray-600">생성 트랜잭션:</span>
                              <a
                                href={`https://sepolia.etherscan.io/tx/${campaign.blockchainTransactionHash}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="ml-2 text-[#009591] hover:underline text-xs"
                              >
                                {campaign.blockchainTransactionHash.substring(0, 10)}...
                              </a>
                            </div>
                          )}
                        </>
              ...
              <Card className="sticky top-24">
                <CardContent className="p-6">
                  <div className="text-center mb-6">
                    <p className="text-2xl font-bold text-[#009591] mb-2">
                      {currentAmount.toLocaleString()}원
                    </p>
                    <p className="text-sm text-gray-600 mb-4">
                      목표금액 {campaign.targetAmount.toLocaleString()}원 중 {progressPercentage.toFixed(1)}%
                    </p>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-center text-sm mb-4">
                    <div>
                      <p className="font-semibold">{campaign.donorCount}명</p>
                      <p className="text-gray-600">참여자</p>
                    </div>
                    <div>
                      <p className="font-semibold">{daysLeft}일</p>
                      <p className="text-gray-600">남음</p>
                    </div>
                  </div>
                        ...
                        <Link key={related.id} href={`/campaign/${related.id}`}>
                          <div className="flex space-x-4 p-4 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer border border-gray-100">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-600 mb-1">{related.organizer || related.creatorName}</p>
                              <p className="text-sm text-gray-500 mb-3">{relatedCategoryTranslated}</p>
                              <div className="flex items-center justify-between">
                                <span className="text-sm text-[#009591] font-semibold">
                                  {related.currentAmount.toLocaleString()}원
                                </span>
                                <span className="text-sm text-gray-500">{relatedProgressPercentage.toFixed(1)}%</span>
                              </div>
                            </div>
                          </div>
                        </Link>
